...
class Formula: 
    ...
    def substitute(self, var, val):
        ...
        tree = self.tree
        return Formula(tree.replace_subtree(Tree(root = var), val))
    ...
    def __call__(self, *value_list, **value_dict):
        if value_dict == {}:
            ...
            for var,val in zip(\
                ...
                value_dict[var] = val
            return Formula._calculate(self.tree, value_dict)
        elif value_list == ():
    ...
    def _calculate(tree, value_dict):
        if tree.datum[0] == 'op':
            args = [Formula._calculate(c, value_dict) for c in tree.children]
...
def pop_operator(operator, operand):
    ...
    if top[1] in ['+', '*', '/', '^', '-']:
        ...
        if isinstance(arg1, Tree):
            if (arg1.children == [] and arg1.root.datum[0] == 'num'):
                operand.push(Tree(root = ('num', '-%s'%arg1.root.datum[1])))
            else:
...
def push_operator(operator, operand, op):
    if not operator.is_empty():
        top = operator.top()
        while OPERATOR_PRIORITY[top[1]] > OPERATOR_PRIORITY[op[1]]:
            pop_operator(operator, operand)
            top = operator.top()
            if top is None:
                break
